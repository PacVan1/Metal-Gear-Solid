	...
	mGUI(this)
{
	...
	Lights& init = LIGHTS; // init lights
	...
	mPlayer->SetLocalPosition(vec3(-150.0f, 200.0f, 0.0f));
...
void GameOpeningState::Enter(Game& g)
{
	GetWindow().EnableCursor();
}

void GameOpeningState::Exit(Game& g)
{
...
void GameMenuState::Tick(Game& g, float const dt)
{
	if (Inputs::ToggleMenu()) g.mSM.SwitchState(g.mPlayingState);
}

void GameMenuState::RenderGUI(Game& g)
{
