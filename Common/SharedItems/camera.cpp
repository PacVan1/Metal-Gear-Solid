...
mat4 Camera::CalcOrtho() const
{
	return glm::ortho(0.0f, 10.0f, 10.0f, 0.0f, CAMERA_NEAR_PLANE, CAMERA_FAR_PLANE);
}
...
void OrbitalCamera::Tick(float const)
{
	if (!mPlayer) return;
...
void FirstPersonCamera::Tick(float const dt)
{
	...
	{
		mAngle		+= dt * 700.0f;
		mPosition.y += cos(radians(mAngle)) * 0.2f;
	}
}
